...
public sealed class ObsControlV220
{
    ...
    private sealed class Session : IAsyncDisposable
    {
        ...
        public async Task<JsonObject?> RequestAsync(string requestType, JsonObject? requestData = null)
        {
            ...
            {
                JsonObject? message;
                try { message = await ReceiveObjectAsync(cts.Token); }
                catch (OperationCanceledException ex) { throw new TimeoutException($"OBS request '{requestType}' timed out.", ex); }
